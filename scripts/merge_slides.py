...
def bench_fig(det, keys, path, title):
    ...
    for k in keys:
        ...
        if "trajectory" in d:
            ...
            ax.plot(d["trajectory"]["t"], I, label=LAB[k], color=COL[k], lw=2.5)
...
# ---------------------------------------------------------------- deck (fusion)
BASE = Path(r"C:\Users\12354657G\Desktop\projet integrer\presentation_complete.pptx")
prs = Presentation(str(BASE))                       # phase 1 (25 slides) comme base
SW, SH = prs.slide_width, prs.slide_height
_SCALE = SW / _Inches(13.333)                       # mes coords sont en 13.333 -> échelle


def Inches(v):
    return Emu(int(_Inches(v) * _SCALE))
...
BLANK = prs.slide_layouts[0]                         # layout vierge de la phase 1
print("Base: %d slides, %.2fx%.2f, scale=%.3f" % (len(prs.slides._sldIdLst), SW/914400, SH/914400, _SCALE))
...
out = Path(r"C:\Users\12354657G\Desktop\projet integrer\presentation_complete_avec_resultats.pptx")
prs.save(str(out))
